...
export const nameTransformer = createXMLTransformer({})((_, { textContent }) => {
    if (!textContent) {
        return IGNORE_NODE;
    }
    const indexedNameMatch = textContent.match(/^([\s\S]+?)(?:\s?\((\d{1,3})\))?$/);
    if (!indexedNameMatch) {
        throw new XMLTransformerError(`text "${textContent}" did not match regex pattern`);
    }
    return {
...
export const artistTransformer = createXMLTransformer({
    ...
    const { id, name: { name, nameIndex, originalName }, join, role, tracks, anv = [], } = childResultsToSingular(withRequiredProperties(childResults, "id", "name"), "id", "name", "join", "role", "tracks");
    return {
        id,
        name,
        nameIndex,
...
export const artistsTransformer = createListTransformer("artist", artistTransformer);
